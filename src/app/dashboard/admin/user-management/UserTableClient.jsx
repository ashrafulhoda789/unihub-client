...
export default function UserTableClient({ users }) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900/60 border border-slate-800 p-4 rounded-3xl backdrop-blur-xl shadow-xl">
                {/* Search Input */}
                <div className="relative w-full sm:w-72">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-950 border border-slate-700/80 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-all"
                    />
                </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                    ...
                    <select
                        ...
                        className="bg-slate-950 border border-slate-700/80 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer capitalize"
                    >
                                    ...
                                    return (
                                        <tr key={userId} className="hover:bg-slate-800/30 transition-colors">
                                            ...
                                            <td className="py-4 px-6">
                                                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-semibold capitalize ${currentRole === "admin"
                                                        ? "bg-rose-500/10 text-rose-400 border border-rose-500/20"
                                                        : currentRole === "faculty"
                                                            ? "bg-cyan-500/10 text-cyan-400 border border-cyan-500/20"
                                                            : "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20"
                                                    }`}>
                                                    {currentRole === "admin" ? <Shield className="w-3 h-3" /> : <UserCheck className="w-3 h-3" />}
                                                    {currentRole}
                                                </span>
                                            </td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-6 relative">
                        ...
                        <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-4 text-xs space-y-2 text-slate-300">
                            <p><span className="text-slate-500">User:</span> <strong className="text-white">{confirmModal.userName}</strong></p>
